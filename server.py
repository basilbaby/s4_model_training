...
<body>
    ...
    <div class="content">
        <div class="left-panel">
            <div class="training-params">
                ...
                <p><span class="param-label">Optimizer:</span> <span id="optimizer">-</span></p>
                <p><span class="param-label">Learning Rate:</span> <span id="learning_rate">-</span></p>
                <p><span class="param-label">Batch Size:</span> <span id="batch_size">-</span></p>
                <p><span class="param-label">Epochs:</span> <span id="total_epochs">-</span></p>
                <p id="momentum_container" style="display: none">
                    <span class="param-label">Momentum:</span> <span id="momentum">-</span>
                </p>
            </div>
            <div class="model-stats">
                <div class="model-card">
                    <h3>Model 1 <span class="model-type" id="model1_type">-</span></h3>
                    <p>Kernels: <span id="model1_kernels">-</span></p>
                    <p>Loss: <span id="model1_loss">-</span></p>
                    <p>Accuracy: <span id="model1_accuracy">-</span></p>
                </div>
                <div class="model-card">
                    <h3>Model 2 <span class="model-type" id="model2_type">-</span></h3>
    ...
    <script>
        ...
        function updateResults() {
            fetch('/logs/training_log.json')
                ...
                .then(data => {
                    ...
                    // Update previous runs - limit to last 3
                    fetch('/logs')
                        ...
                        .then(text => {
                            ...
                            const files = Array.from(doc.querySelectorAll('a'))
                                ...
                                .sort()
                                .reverse()
                                .slice(0, 3);  // Limit to last 3 runs
                            ...
                            previousRunsDiv.innerHTML = '<h3>Previous Runs (Last 3)</h3>';
                            files.forEach(runId => {
                                const runDiv = document.createElement('div');
                                runDiv.className = 'previous-run-item';
                                runDiv.innerHTML = `
        ...
        function updateStats() {
            const now = Date.now();
            fetch('/logs/training_log.json?' + now)  // Add timestamp to prevent caching
                .then(response => response.json())
                .then(data => {
                    // Update training parameters
                    if (data.training_params) {
                        const params = data.training_params;
                        
                        // Update optimizer (convert to uppercase for display)
                        document.getElementById('optimizer').textContent = 
                            (params.optimizer || 'adam').toUpperCase();
                        
                        // Update learning rate (use provided value or default)
                        document.getElementById('learning_rate').textContent = 
                        ...
                        // Update epochs
                        document.getElementById('total_epochs').textContent = 
                            params.epochs;

                        // Handle momentum display for SGD
                        const momentumContainer = document.getElementById('momentum_container');
                        if (params.optimizer === 'sgd' && params.momentum !== null) {
                            momentumContainer.style.display = 'block';
                            document.getElementById('momentum').textContent = params.momentum;
                        } else {
                    ...
                    // Update device and time information
                    document.getElementById('device').textContent = data.device;
                    document.getElementById('elapsed_time').textContent = data.elapsed_time || '-';
                    document.getElementById('current-run-id').textContent = data.run_id;
